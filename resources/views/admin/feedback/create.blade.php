...
<div class="container-fluid">
  <h1 class="mt-4">Tambah Feedback Pelanggan</h1>
      ...
      <form action="{{ route('admin.feedback.store') }}" method="POST">
        ...
        <div class="mb-3">
          ...
          <select class="form-select @error('skor') is-invalid @enderror" id="skor" name="skor" required>
            ...
            @for ($i = 1; $i <= 5; $i++)
              <option value="{{ $i }}" {{ old('skor') == $i ? 'selected' : '' }}>{{ $i }} Bintang</option>
              @endfor
          </select>
          @error('skor')
          <div class="invalid-feedback">{{ $message }}</div>
        ...
        <div class="mb-3">
          ...
          <textarea class="form-control @error('komentar') is-invalid @enderror" id="komentar" name="komentar" rows="3" required>{{ old('komentar') }}</textarea>
          @error('komentar')
          <div class="invalid-feedback">{{ $message }}</div>
          @enderror
        </div>
        ...
        <div class="mb-3">
          ...
          <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
            ...
            <option value="Sedang Ditangani" {{ old('status') == 'Sedang Ditangani' ? 'selected' : '' }}>Sedang Ditangani</option>
